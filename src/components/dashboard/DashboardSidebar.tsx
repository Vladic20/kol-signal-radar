...
const SidebarItem = ({ href, icon: Icon, label, isActive, badge }: SidebarItemProps) => (
  <Link to={href} className="block">
    <Button 
      variant="ghost" 
      className={`w-full justify-start space-x-3 h-11 px-4 text-left transition-all duration-200 cursor-pointer relative group ${
        isActive 
    ...
    >
      ...
      {badge && (
        <span className="absolute right-3 bg-neon-purple text-white text-xs px-1.5 py-0.5 rounded-full animate-pulse">
          {badge}
        </span>
      )}
    </Button>
      ...
      <nav className="flex-1 overflow-y-auto">
        ...
        <div className="p-2">
          ...
          <div className="px-2 py-2">
            <Button className="w-full bg-gradient-to-r from-neon-purple to-neon-blue hover:opacity-90 text-sm h-10 transition-all hover:scale-105 shadow-lg">
